...
/**
 * @fileoverview Email Sender Utility — BB Wings Management System
 * @description Utilidad para enviar correos electrónicos de credenciales y notificaciones por SMTP.
 * @version 1.0.0
 */
...
export async function sendEmail({ to, subject, html, text }: SendEmailParams) {
  // Generar versión en texto plano si no se provee (Gmail penaliza emails que solo tienen HTML sin alternativa de texto)
  ...
  const mailOptions = {
    from: `"BB Wings" <${process.env.SMTP_USER || ""}>`,
    to,
    subject,
    html,
    text: plainText,
    // Cabeceras adicionales para asegurar entrega transaccional
    headers: {
      "X-Priority": "3", // Prioridad Normal
      "X-Mailer": "Nodemailer",
      "Message-ID": `<${Date.now()}.${Math.random().toString(36).substring(2, 11)}@bbwings.com>`
    }
  };
  ...
  try {
    ...
    console.log(`[SMTP] Correo enviado con éxito. MessageId: ${info.messageId}`);
    return info;
  } catch (error) {
  ...
  return `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #0c0c0c; color: #ffffff; padding: 40px 20px; border-radius: 12px; max-width: 600px; margin: 0 auto; border: 1px solid rgba(255,255,255,0.08);">
      <div style="text-align: center; margin-bottom: 30px;">
        ...
        <p style="color: rgba(255,255,255,0.5); font-size: 14px; margin-top: 5px; text-transform: uppercase; letter-spacing: 1px;">Gestión de Cuentas</p>
      </div>
      ...
      <div style="background-color: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06); padding: 30px; border-radius: 10px;">
        ...
        <p style="font-size: 15px; color: rgba(255,255,255,0.7); line-height: 1.6;">
          Se ha creado tu cuenta de usuario para ingresar al panel administrativo de <strong>BB Wings</strong>. A continuación, encontrarás tus credenciales de acceso temporal:
        </p>
        ...
        <div style="background-color: rgba(234, 88, 12, 0.05); border-left: 4px solid #ea580c; padding: 20px; border-radius: 6px; margin: 25px 0;">
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr>
              <td style="padding: 6px 0; font-size: 14px; color: rgba(255,255,255,0.4); font-weight: 700; text-transform: uppercase;">Contraseña:</td>
        ...
        <p style="font-size: 14px; color: rgba(255,255,255,0.5); line-height: 1.5; margin-bottom: 25px;">
          * Por motivos de seguridad, te recomendamos cambiar esta contraseña temporal una vez que inicies sesión en tu perfil administrativo.
        </p>
